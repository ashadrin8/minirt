...
/*   Updated: 2026/01/08 17:36:44 by ashadrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	validate_store_ambient(t_parsing *p, t_scene *scene)
{
	...
	if (p->position == 2)
	{
		if (validate_and_store_color(p->parameter, &scene->ambient.colors) == ERROR)
			return (ERROR);
	}
	return (SUCCESS);
}
...
int	validate_store_camera(t_parsing *p, t_scene *scene)
{
	...
	if (p->position == 1)
	{
		if (validate_and_store_coordinates(p->parameter, &scene->camera.coords) == ERROR)
			return (printf("error here, hahaha\n"), ERROR);
	}
	if (p->position == 2)
	{
		if (validate_and_store_vector(p->parameter, &scene->camera.vector) == ERROR)
			return (printf("oops, error\n"), ERROR);
	}
	if (p->position == 3)
	{
		printf("i was on fov\n");
...
int	validate_store_light(t_parsing *p, t_scene *s)
{
	...
	if (p->position == 1)
	{
		if (validate_and_store_coordinates(p->parameter, &s->light.coords) == ERROR)
			return (ERROR);
	}
	if (p->position == 2)
	{
		if (ascii_to_double(p->parameter, &brightness) == ERROR)
			return (printf("no light on the horizonnn\n"), ERROR);
...
int	validate_store_sphere(t_parsing *p, t_scene *s)
{
	...
	if (p->position == 1)
	{
		if (validate_and_store_coordinates(p->parameter, &s->last_sphere->center) == ERROR)
			return (ERROR);
	}
	if (p->position == 2)
	{
...
int	validate_store_plane(t_parsing *p, t_scene *s)
{
	if (p->position == 4)
		return (ERROR);
	if (p->position == 1)
		return (validate_and_store_coordinates(p->parameter, &s->last_plane->point));
	if (p->position == 2)
		return (validate_and_store_vector(p->parameter, &s->last_plane->vector));
	if (p->position == 3)
		return (validate_and_store_color(p->parameter, &s->last_plane->color));
	return (SUCCESS);
}
...
int	validate_store_cylinder(t_parsing *p, t_scene *s)
{
	if (p->position == 6)
		return (ERROR);
	if (p->position == 1)
		return (validate_and_store_coordinates(p->parameter, &s->last_cylinder->center));
	if (p->position == 2)
		return (validate_and_store_vector(p->parameter, &s->last_cylinder->vector));
	if (p->position == 3)
	{
